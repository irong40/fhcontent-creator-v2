...
/**
 * GET  /api/analytics/pull  — daily Vercel cron (see vercel.json)
 * POST /api/analytics/pull  — manual trigger from the dashboard
 *
 * Pulls engagement metrics from Blotato's analytics API for all published
 * content pieces and stores point-in-time snapshots in performance_metrics.
 *
...
function mapMetrics(m: Record<string, number | null | undefined>) {
...
async function runAnalyticsPull() {
    ...
    if (!pieces || pieces.length === 0) {
        return NextResponse.json({ message: 'No published pieces in window', snapshots: 0 });
    }
        ...
        for (const entry of Object.values(platforms) as Array<PlatformStatus | undefined>) {
            ...
            try {
                ...
                if (typeof url === 'string' && url.length > 0) {
                    entry.post_url = url;
                    dirty = true;
                    counters.urlsResolved++;
                }
            } catch {
                counters.urlResolutionErrors++;
            }
            await sleep(150);
        }
    ...
    for (const piece of pieces as PieceRow[]) {
        ...
        for (const [platform, entry] of Object.entries(platforms) as Array<[string, PlatformStatus | undefined]>) {
            ...
            if (!listId) { counters.unmatched++; continue; }
            ...
            try {
                ...
                if (!hasSignal) { counters.noMetricsYet++; continue; }
                ...
                if (insertError) counters.insertErrors++;
                else counters.snapshots++;
            } catch {
