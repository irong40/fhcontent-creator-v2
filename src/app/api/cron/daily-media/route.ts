...
import { generatePhotoCascade } from '@/lib/photo-provider';
    ...
    try {
        ...
        for (const [, personaTopics] of topicsByPersona) {
            ...
            for (const topicRow of personaTopics) {
                ...
                // archival → generated → satori template fallback. Every
                // photographic rung is audited against the persona subject
                // constraint; the template fallback (no people) bypasses it
                // legitimately.
                //
                // The Imagen rung was removed 2026-07-28: imagen-4.0-* returns 404
                // "no longer available to new users", and it burned two wasted
                // round-trips per image (382 attempts, 382 errors) before falling
                // through. The generated rung now goes through
                // generatePhotoCascade (gemini-3.1-flash-image, then gpt-image-1),
                // because on 2026-07-30 the sole remaining provider was also dead
                // — zero OpenAI credits — and every image silently became text.
                const imageLadderDeps: SlideLadderDeps = {
                    generatePhoto: (prompt) =>
                        generatePhotoCascade(prompt, { log: (m) => console.log(m) }),
